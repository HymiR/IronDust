...
namespace irondust
{
    namespace gl
    {
        ...
        {
            if(image->getDepth() > 1) {
                LOG_ERROR << "can only handle 2D Images, but "
                            << image->getName()
                            << " has a depth of "
                            << image->getDepth()
                            << std::endl;
            ...
            Texture2D* tex = new Texture2D(
                    image->getName(),
                    glm::uvec2(image->getWidth(), image->getHeight()),
                    textureId, image->getFormat());
        ...
        Texture2D::Texture2D(
                ...
                const glm::uvec2& dim,
                const GLuint textureId, GLint format)
            : Texture(name, textureId, format, GL_TEXTURE_2D)
            , dim(dim)
        {
        }
